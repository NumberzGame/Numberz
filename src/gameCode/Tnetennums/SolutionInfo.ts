...
import { combinations, Grade, Op, opsAndLevelsResults, Result, Seed, SolutionForm } from './Core';
...
function getCombinedSolutionForm(subForm1: SolutionForm, subForm2: SolutionForm): SolutionForm {
...
export class SolutionInfo {
  ...
  *get_solutions_extended_by_sub_sol(
    goal: Result,
    sub_sol: SolutionInfo,
    symbols_and_grades: Record<Op, Grade>
  ): IterableIterator<SolutionInfo> {
    yield* SolutionInfo.getCombinedFromSubSolutions(goal, this, sub_sol, symbols_and_grades);
  }
  ...
  *get_solutions_extended_by_seed(
    ...
    yield* this.get_solutions_extended_by_sub_sol(goal, trivial, symbols_and_grades);
  }
  ...
  static *get_triples(three_seeds: Seed[]): IterableIterator<SolutionInfo> {
    ...
    for (const two_seeds of combinations(2, three_seeds)) {
      ...
      for (const [_pair_symbol, [pair_result, pair_level]] of Object.entries(
        ...
        //# pair = op(*two_seeds)
        const pair_sol = SolutionInfo.get_pairs(pair_result, two_seeds[0], two_seeds[1], {
